# Remote docker CLI
* For connecting and using docker daemon socket remotely, follow this instructions:  
```
...
```
and then we can log into another server and run the docker command with `-H`:  
```
docker -H <example:192.168.1.1:2375> images
```
